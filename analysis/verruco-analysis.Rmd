...
scaled <- scale_reads(dupmerge.subset,min(sample_sums(dupmerge.subset)))   ### Scaled to 10,808 (minumum sample)
...
################# TIME TO CREATE STACKED BAR PLOT OF Lake_TypeS #################
...
Lake_Type.Ver[1,] <- (Lake_Type.Ver[1,]/54)
    ## Averaging Estuary
Lake_Type.Ver[2,] <- (Lake_Type.Ver[2,]/117)
    ## Averaging Inland
Lake_Type.Ver[3,] <- (Lake_Type.Ver[3,]/45)
    ## Averaging Laurentian
...
ggplot(data = Lake_Type.Ver, aes(x = Lake_Type, y = Otu000064)) +
...
barplot(sort(taxa_sums(Ver.rel),TRUE)[1:20]/nsamples(Ver.rel),las=2,cex.axis=.7)
    ## 1 = OPB35_soil_Lake_Type
    ## 2 = OPB35_soil_Lake_Type
...
Vdeseq <- subset_taxa(verr.dupmerge, Phylum == "Verrucomicrobia")
  # We want to use the RAW COUNTS
pick.Ver<- colnames(otu_table(Ver))
...
ggplot(source, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
...
ggplot(watercol, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer (palette="Set1") +
...
ggplot(Vbs, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vst, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vel, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vei, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vli, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vss, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vspf, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
ggplot(Vsuf, aes(x=Species, y=log2FoldChange, color=Class)) + geom_point() +
  scale_color_brewer(palette="Set1") +
...
write.csv(Lake_Type.split[5,], file="Ecology_Otu2", row.names=FALSE, quote=FALSE)
```

### Create updated phyloseq object with EcoID
```{r ecoid.phyloseq}
##### Create new phyloseq object with EcoIDs
eco.label<- read.csv("Ecology_Otu2", header=F)
...
color.id <- function(x) {
  ifelse(grepl("OPB35_soil_Lake_Type",x),"#e41a1c",
  ifelse(grepl("Opitutae",x), "#377eb8",
  ifelse(grepl("Spartobacteria",x), "#4daf4a",
  ifelse(grepl("S-BQ2-57_soil_Lake_Type",x), "#984ea3",
...
write.csv(color.all$label,file="all.ver.color1",row.names=FALSE,quote=FALSE)
...
write.csv(ecoid.all$itol.label,"EcoID.IToL.label2", row.names=FALSE, quote=FALSE)
```
...
write.tree(phy_tree(only.sig), file="ver.sig.tre2")
...
```{r sig.itol.abund}
refit.tree <- read.newick(file="ver.sig.tre.refit")
refit.otus <- refit.tree$tip.label

remove.tree <- read.newick(file="ver.sig.tre4")
remove.otus <- remove.tree$tip.label

refit.unique <- setdiff(refit.otus, remove.otus)
  # refit.otus that are not in remove.otus
remove.unique <- setdiff(remove.otus, refit.otus)
  # remove.otus that are not in refit.otus

refit.phy <- prune_taxa(names(taxa_sums(Vdeseq.pick)) %in% refit.otus, x=Vdeseq.pick)
refit.phy <- merge_phyloseq(refit.phy, refit.tree)
#write.tree(phy_tree(refit.phy), file="refit.tre")
refit.rel <- transform_sample_counts(refit.phy, function(x){x/sum(x)})
summary(taxa_sums(refit.rel))
# Create rank abundance of OTUs
barplot(sort(taxa_sums(refit.rel),TRUE)/nsamples(refit.rel),las=2,cex.axis=.7)
# Create rank abundance of OTUs
barplot(sort(taxa_sums(refit.rel),TRUE)[1:60]/nsamples(refit.rel),las=2,cex.axis=.7)



refit.relabund <- prune_taxa(names(taxa_sums(Ver.rel)) %in% refit.otus, x=Ver.rel)


refit.unique.phy <- prune_taxa(names(taxa_sums(Vdeseq.pick)) %in% refit.unique, x=Vdeseq.pick)
