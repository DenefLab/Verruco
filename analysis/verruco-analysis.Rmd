...
pairs(lau.df[21:35])
  # Only use DO, TP, PON, DOC, & PAR

pairs(est.df[21:37])

  # Only use ORP, Chl, BGA, SO4, NH3, TKN, TP, & DO
png("test.png", width = 80, height = 40, units = "cm", res = 600)
pairs(inl.df[21:26])
dev.off()
  # Only use NO3, TP, & NH4

# Trim dataframes so that only uncorrelated factors are left
## Laurentian
lau.trim <- lau.df[,-23]
lau.trim <- lau.trim[,-25]
lau.trim <- lau.trim[,-25]
lau.trim <- lau.trim[,-25]
lau.trim <- lau.trim[,-21]
lau.trim <- lau.trim[,-22]
lau.trim <- lau.trim[,-23]
lau.trim <- lau.trim[,-25]
lau.trim <- lau.trim[,-25]
lau.trim <- lau.trim[,-25]
...
## Estuary
est.trim <- est.df[,-22]
est.trim <- est.trim[,-27]
est.trim <- est.trim[,-28]
est.trim <- est.trim[,-30]
est.trim <- est.trim[,-31]
est.trim <- est.trim[,-32]
est.trim <- est.trim[,-22]
est.trim <- est.trim[,-22]
est.trim <- est.trim[,-21]

## Inland
inl.trim <- inl.df[,-21]
inl.trim <- inl.trim[,-22]
inl.trim <- inl.trim[,-23]
...
### Laurentian
xyplot(log(Abundance) ~ DO.Probe..mg.L., lau.trim)
lau.DO <- lm(DO.Probe..mg.L.~ log(Abundance), lau.trim)
summary(lau.DO)
...
## Now let's test the environmental factors that I had trimmed
xyplot(log(Abundance) ~ DOC.mg.L, lau.trim)
lau.DOC <- lm(DOC.mg.L~log(Abundance), lau.trim)
summary(lau.DOC)
confint(lau.DOC, level=0.95)
  # Negative R2

xyplot(log(Abundance) ~ TP.ug.L, lau.trim)
lau.TP <- lm(TP.ug.L~log(Abundance), lau.trim)
summary(lau.TP)
confint(lau.TP, level=0.95)
  # Not linear

xyplot(log(Abundance) ~ PON.mg.L, lau.trim)
lau.PON <- lm(PON.mg.L~log(Abundance), lau.trim)
summary(lau.PON)
confint(lau.PON, level=0.95)
  # Not linear

xyplot(log(Abundance) ~ PAR, lau.trim)
lau.PAR <- lm(PAR~log(Abundance), lau.trim)
summary(lau.PAR)
...
### Estuary
xyplot(log(Abundance) ~ ORP..mV., est.trim)
est.ORP <- lm(ORP..mV.~log(Abundance), est.trim)
summary(est.ORP)
...
xyplot(log(Abundance) ~ Chl.Lab..ug.L., est.trim)
est.Chl <- lm(Chl.Lab..ug.L.~log(Abundance), est.trim)
summary(est.Chl)
confint(est.Chl, level=0.95)
  # Linear

xyplot(log(Abundance) ~ BGA.cells.mL, est.trim)
est.BGA <- lm(BGA.cells.mL~log(Abundance), est.trim)
summary(est.BGA)
confint(est.BGA, level=0.95)
  # Not linear

xyplot(log(Abundance) ~ SO4.mg.L, est.trim)
est.SO4 <- lm(SO4.mg.L~log(Abundance), est.trim)
summary(est.SO4)
confint(est.SO4, level=0.95)
  # Linear

xyplot(log(Abundance) ~ NH3.mg.L, est.trim)
est.NH3 <- lm(NH3.mg.L~log(Abundance), est.trim)
summary(est.NH3)
confint(est.NH3, level=0.95)
  # Not linear

xyplot(log(Abundance) ~ TKN.mg.L, est.trim)
est.TKN <- lm(TKN.mg.L~log(Abundance), est.trim)
summary(est.TKN)
confint(est.TKN, level=0.95)
  # Negative R2

xyplot(log(Abundance) ~ TP.ug.L, est.trim)
est.TP <- lm(TP.ug.L~log(Abundance), est.trim)
summary(est.TP)
confint(est.TP, level=0.95)
  # Negative R2

xyplot(log(Abundance) ~ DO.probe.mg.L, est.trim)
est.DO <- lm(DO.probe.mg.L~log(Abundance), est.trim)
summary(est.DO)
...
## Now let's test the environmental factors that I had trimmed
xyplot(log(Abundance) ~ NO3.mg.L, inl.trim)
inl.NO3 <- lm(NO3.mg.L~log(Abundance), inl.trim)
summary(inl.NO3)
confint(inl.NO3, level=0.95)
  # Negative R2

xyplot(log(Abundance) ~ TP.ug.L, inl.trim)
inl.TP <- lm(TP.ug.L~log(Abundance), inl.trim)
summary(inl.TP)
confint(inl.TP, level=0.95)
  # Linear?

xyplot(log(Abundance) ~ NH4.ug.L, inl.trim)
inl.NH4 <- lm(NH4.ug.L~log(Abundance), inl.trim)
summary(inl.NH4)
...
#write.tree(bs.tre, "treeBS")

# Compare IToL tree with bootstrap tree
bs.tre <- read.newick(file="treeBS")
