...
library(car)
theme_set(theme_bw())
windowsFonts(Times=windowsFont("Times New Roman"))set.seed(1)
...
Ver.box.LM.plot1 <- ggplot(Ver.box.LM, aes(x=Station, y= Relative_Abundance, 
  ...
  ggtitle("Lake Michigan Survey Samples") +
...
adonis(Ver.wUF ~ Lake_Type + Source + Fraction + Season + Depth , data=sampledf)

adonis(Ver.wUF ~ Depth + Lake_Type + Source + Fraction + Season, data=sampledf)



tiff("test.tiff", width = 17.4, height = 12, units = "cm", res = 325)
plot_ordination(physeq=Ver.unifrac, ordination=wUF.ord) +
  geom_point(size=2, aes(color=Fraction, shape=Source))
dev.off()
