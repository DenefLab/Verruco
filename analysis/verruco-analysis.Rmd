...
### UNSTACKED BAR PLOT OF Lake_Type
```{r}
...
# First average Laurentian
phy.otu[1,] <- (phy.otu[1,]/35)
# Next average Estuary
phy.otu[2,] <- (phy.otu[2,]/65)
# Then average Inland
phy.otu[3,] <- (phy.otu[3,]/126)

# Now back into a phyloseq object
tphy.otu <- t(phy.otu)
phy.otu <- otu_table(tphy.otu, taxa_are_rows = T)
phy.tax <- tax_table(phy.Lake_Type)

phy.samp <- data.frame(sample_data(phy.Lake_Type))
phy.samp$Lake_Type <- rownames(phy.samp)
phy.samp <- sample_data(phy.samp)

# This makes a new phyloseq object with Lake Types
...
phy.Lake_Type.merge <- prune_taxa(taxa_sums(phy.Lake_Type.merge) > 0,
                                  phy.Lake_Type.merge)

# Separate out Lake Type data
phy.lau <- subset_samples(phy.Lake_Type.merge, Lake_Type=="Laurentian")
phy.est <- subset_samples(phy.Lake_Type.merge, Lake_Type=="Estuary")
phy.inl <- subset_samples(phy.Lake_Type.merge, Lake_Type=="Inland")

# Calculate mean & standard deviation by Lake_Type
# You'll use this later to add error bars to your unstacked bar plot
phy.df <- psmelt(phylum.99)
phy.sum <- ddply(phy.df, c("Lake_Type", "Phylum"),
                 summarise,
                 Mean = mean(Abundance),
                 Median = median(Abundance),
                 SD = sd(Abundance, na.rm = T),
                 SE = se(Abundance, na.rm=T))
...
# Break up summaries by Lake_Type
phy.lau.sum <- phy.trim[1:8,]
phy.est.sum <- phy.trim[9:16,]
phy.inl.sum <- phy.trim[17:24,]
...
##### Laurentian #####
phy.lau.long <- psmelt(phy.lau)
# Removing phylum "unclassified" since that's not in phy.lau.long
phy.lau.sum <- phy.lau.sum[-12,]
# Add standard deviation column to our ggplot data
phy.lau.long$SD <- sort(phy.lau.sum$SD, decreasing=T)
# Add median column to our ggplot data
phy.lau.long$Median <- sort(phy.lau.sum$Median, decreasing=T)
# Add standard error column to our ggplot data
phy.lau.long$SE <- sort(phy.lau.sum$SE, decreasing=T)


#tiff("stackbar.Lake_Type.tiff", width = 7, height = 8, units = "in", res = 500)
ggplot(phy.lau.sum, aes(x=Phylum, y=Median, fill=Phylum)) +
          geom_bar(stat="identity", position="dodge", color="black") +
          #scale_y_continuous(labels = percent_format()) +
          scale_fill_manual(values = phylum.colors, name="Phylum") +
  geom_errorbar(aes(ymax=Median+SE, ymin=Median-SE), width=0.3) + 
...
##### Estuary #####
phy.est.long <- psmelt(phy.est)


#tiff("stackbar.Lake_Type.tiff", width = 7, height = 8, units = "in", res = 500)
x=reorder(Phylum, order(Abundance, decreasing=TRUE))

ggplot(phy.est.sort, aes(x=reorder(Phylum, order(Abundance, decreasing=TRUE)),
...
##### Inland #####
phy.inl.long <- psmelt(phy.inl)
phy.inl.sort <- arrange(phy.inl.long, Phylum)

#tiff("stackbar.Lake_Type.tiff", width = 7, height = 8, units = "in", res = 500)
ggplot(phy.inl.sort, aes(x=reorder(Phylum, order(Abundance, decreasing=TRUE)), y=Abundance, fill=Phylum)) +
          geom_bar(stat="identity", position="dodge", color="black") +
          #scale_y_continuous(labels = percent_format()) +
          scale_fill_manual(values = phylum.colors, name="Phylum") + 
          theme(axis.title.x = element_text(size=16, face = "bold"),
