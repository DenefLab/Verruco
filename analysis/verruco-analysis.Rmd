...
ann_LM <- data.frame(lab=c(" ", " ", "+"),
                       Station = factor(c("Buoy", "Shore", "Far"),
                                          levels = c("Buoy", "Shore", "Far")))
Ver.box.LM.plot1 <- ggplot(Ver.box.LM, aes(x=Station, y= Relative_Abundance, 
                                           fill = Station)) +
  geom_boxplot(position=position_dodge(1)) +
  ggtitle("Lake Michigan Cruise Samples") +
  ...
  geom_text(data = ann_LM, aes(label=lab), x=c(1,1,3), y=c(16,16,18), size=4, color="red")

# Ver.box.LM.plot2 <- ggplot(Ver.box.LM, aes(x=Lake_Type, y= Relative_Abundance, 
#                                           fill = Lake_Type)) +
#   geom_boxplot(position=position_dodge(1)) +
#   ggtitle("Lake Michigan Cruise 2013") +
#   theme(axis.title.x = element_text(face="bold", size=8),
#         axis.text.x = element_text(angle=0, colour="black", size=6),
#         axis.text.y = element_text(colour="black", size=6),
#         axis.title.y = element_text(face="bold", size=8),
...
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.LM, 
  ...
  # p-value = 0.004086
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.LM,
             subset=Station!="Shore")
  # Buoy vs. Far (Muskegon vs. LM)
  # p-value = 0.01045
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.LM,
             subset=Station!="Far")
  # Buoy vs. Near (Muskegon vs. LM)
  # p-value = 0.3881
kruskal.test(formula = Relative_Abundance ~ Lake_Type, data = Ver.box.LM)
  # Musk vs. LM
  # p-value = 0.0596
#t.test(formula = Relative_Abundance ~ Lake_Type, data = Ver.box.LM)
  # p-value = 0.06987
#t.test(formula = Relative_Abundance ~ Lake_Type, data = Ver.box.LM, 
#       alternative = "less")
  # Lau < Est
  # p-value = 0.03493
Ver.box.LM.pval <- data.frame(c(0.004545, 0.004086, 0.01045, 0.3881, 0.0596))
colnames(Ver.box.LM.pval) <- "pvals"
Ver.box.LM.pval$Comp <- c("All.Station", "Shore.Far", "Buoy.Far", "Buoy.Shore", "Lake.Type")
Ver.box.LM.pval$Bonf <- p.adjust(Ver.box.LM.pval$pvals, method="bonferroni")
Ver.box.LM.pval$BenH <- p.adjust(Ver.box.LM.pval$pvals, method="BH")
# Sig = All.Station, Shore.Far, Buoy.Far
# Nonsig = Buoy.Shore, Lake.Type
...
Ver.box.est.plot <- ggplot(Ver.box.est, aes(x=Station, y=Relative_Abundance,
                                            fill = Study)) +
  geom_boxplot(position=position_dodge(1)) +
  ggtitle("Lake Michigan Cruise vs. Muskegon Lake") +
  theme(axis.title.x = element_text(face="bold", size=8),
        axis.text.x = element_text(angle=0, colour="black", size=6),
        axis.text.y = element_text(colour="black", size=6),
        axis.title.y = element_text(face="bold", size=8),
...
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
  ...
  # p-value = 0.03475
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset=Station!="Inlet")
  # All minus Inlet
  # p-value = 0.01844
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset=Station!="Outlet")
  # All minus Outlet
  # p-value = 0.01521
#t.test(formula = Relative_Abundance ~ Study, data = Ver.box.est)
  # p-value = 0.01307
#t.test(formula = Relative_Abundance ~ Study, data = Ver.box.est, alternative = "less")
  # p-value = 0.006535

## Pairwise Comparisons ##
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station == c("Buoy", "Inlet"))
  # Buoy vs. Inlet
  # p-value = 0.3613

test <- kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station != c("Inlet", "Outlet", "Bottom"))

test2 <- kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station == c("Buoy", "Bear"))
  # Buoy vs. Bear
  # p-value = 0.6242
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station == c("Buoy", "Outlet"))
  # Buoy vs. Outlet
  # p-value = 0.1003
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station == c("Buoy", "Deep"))
  # Buoy vs. Deep
  # p-value = 0.1441


subset.buoy <- which(Ver.box.est$Station == "Buoy")
subset.inl <- which(Ver.box.est$Station == "Inlet")
subset.bear <- which(Ver.box.est$Station == "Bear")
subset.deep <- which(Ver.box.est$Station == "Deep")
subset.out <- which(Ver.box.est$Station == "Outlet")
subset.bu.in <- c(subset.buoy, subset.inl)
subset.bu.be <- c(subset.buoy, subset.bear)
subset.bu.de <- c(subset.buoy, subset.deep)
subset.bu.ou <- c(subset.buoy, subset.out)
t.test(formula = Relative_Abundance ~ Station, data = Ver.box.est[subset.bu.in,])
  # Buoy vs. Inlet
  # p-value = 0.4145
t.test(formula = Relative_Abundance ~ Station, data = Ver.box.est[subset.bu.be,])
  # Buoy vs. Bear
  # p-value = 0.1947
t.test(formula = Relative_Abundance ~ Station, data = Ver.box.est[subset.bu.de,])
  # Buoy vs. Deep
  # p-value = 0.007237
t.test(formula = Relative_Abundance ~ Station, data = Ver.box.est[subset.bu.ou,])
  # Buoy vs. Outlet
  # p-value = 0.01051

### Plot Comparisons ###
png("LM.comp.png", width = 23, height = 5, units = "cm", res = 500)
grid.newpage()
...
print(Ver.box.LM.plot2, vp=viewport(layout.pos.row=1, layout.pos.col=2))
print(Ver.box.est.plot, vp=viewport(layout.pos.row=1, layout.pos.col=3))
dev.off()
...
#tiff("phy.relabund.tiff", width = 17.8, height = 23, units = "cm", res = 600)
...
frac.box <- ggplot(Ver.box.frac, aes(x=Lake_Type, y=Relative_Abundance, fill=Fraction)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=8),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  annotate("text", x= 1, y=41, label = "*", size=5, colour="red")
#dev.off()
...
depth.box <- ggplot(Ver.box, aes(x=Lake_Type, y=Relative_Abundance, fill=Depth)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=8),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  geom_text(data = ann2_text, aes(label=lab), x=c(1,1.33,1.3), y=c(42,42,42), size=3, color="red")
#dev.off()
...
##### Adjust P-Values #####
pvals <- c(4.219e-8, 1.206e-10, 0.8792, 0.0003097, 0.0001692, 0.1491,
           0.0005459, 0.0001081, 0.01748, 6.914e-06, 0.02291, 3.859e-06,
           0.000259, 3.624e-05, 0.6103, 0.241, 0.2177, 0.0009751,
           0.0005545, 0.0007583, 0.3826, 0.000829, 0.8748, 0.07562,
           1.348e-05, 6.914e-06, 0.1037, 9.896e-06, 0.0001089, 0.02212)
...
boxplot.kw.vals$Comp <- c("Lake", "Lake", "Lake", "Frac", "Source", "PF", "PS", "FS", "Frac", "Source", "PF", "PS", "FS", "Frac", "Seas", "Seas", "Est", "Seas", "SuFa", "SpFa", "SpSu", "Depth", "TB", "Depth", "Depth", "WS", "TB", "BS", "TS", "Depth")
...
##### Laur SINE TRANS #####
xyplot(sin(Abundance) ~ DO.Probe..mg.L., lau.df)
lau.DO.sin <- lm(DO.Probe..mg.L.~ sin(Abundance), lau.df)
summary(lau.DO.sin)
confint(lau.DO.sin, level=0.95)
  # Linear

xyplot(sin(Abundance) ~ Temperature..C., lau.df)
lau.temp.sin <- lm(Temperature..C.~sin(Abundance), lau.df)
summary(lau.temp.sin)
confint(lau.temp.sin, level=0.95)
  # Says linear, but R2 is essentially 0

xyplot(sin(Abundance) ~ SiO2.mg.L, lau.df)
lau.Si.sin <- lm(SiO2.mg.L~sin(Abundance), lau.df)
summary(lau.Si.sin)
confint(lau.Si.sin, level=0.95)
  # Linear
...
confint(lau.DOC, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ DOC.mg.L, lau.df)
lau.DOC.sin <- lm(DOC.mg.L~sin(Abundance), lau.df)
summary(lau.DOC.sin)
confint(lau.DOC.sin, level=0.95)
  # R2 = 0.2554
...
confint(lau.TP, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ TP.ug.L, lau.df)
lau.TP.sin <- lm(TP.ug.L~sin(Abundance), lau.df)
summary(lau.TP,sin)
confint(lau.TP.sin, level=0.95)
  # Still not linear
...
confint(lau.PON, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ PON.mg.L, lau.df)
lau.PON.sin <- lm(PON.mg.L~sin(Abundance), lau.df)
summary(lau.PON.sin)
confint(lau.PON.sin, level=0.95)
  # Still not linear
...
confint(lau.PAR, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ PAR, lau.df)
lau.PAR.sin <- lm(PAR~sin(Abundance), lau.df)
summary(lau.PAR.sin)
confint(lau.PAR.sin, level=0.95)
  # R2 = 0.0342
  # Not linear
...
confint(lau.Chl.p, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ Chl.Probe...g.L., lau.df)
lau.Chl.p.sin <- lm(Chl.Probe...g.L.~sin(Abundance), lau.df)
summary(lau.Chl.p.sin)
confint(lau.Chl.p.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(lau.Chl.l, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ Chl.Lab..ug.L., lau.df)
lau.Chl.l.sin <- lm(Chl.Lab..ug.L.~sin(Abundance), lau.df)
summary(lau.Chl.l.sin)
confint(lau.Chl.l.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(lau.TDP, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ TDP.ug.L, lau.df)
lau.TDP.sin <- lm(TDP.ug.L~sin(Abundance), lau.df)
summary(lau.TDP.sin)
confint(lau.TDP.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(lau.TDP.SRP, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ TDP.SRP, lau.df)
lau.TDP.SRP.sin <- lm(TDP.SRP~sin(Abundance), lau.df)
summary(lau.TDP.SRP.sin)
confint(lau.TDP.SRP.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(lau.POC, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ POC.mg.L, lau.df)
lau.POC.sin <- lm(POC.mg.L~sin(Abundance), lau.df)
summary(lau.POC.sin)
confint(lau.POC.sin, level=0.95)
  # Not linear
...
confint(lau.TSS, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ TSS.mg.L, lau.df)
lau.TSS.sin <- lm(TSS.mg.L~sin(Abundance), lau.df)
summary(lau.TSS.sin)
confint(lau.TSS.sin, level=0.95)
  # Not linear
...
confint(lau.fluor, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ Fluorescence.mg.m3., lau.df)
lau.fluor.sin <- lm(Fluorescence.mg.m3.~sin(Abundance), lau.df)
summary(lau.fluor.sin)
confint(lau.fluor.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.ORP, level=0.95)
  # Linear
xyplot(sin(Abundance) ~ ORP..mV., est.df)
est.ORP.sin <- lm(ORP..mV.~sin(Abundance), est.df)
summary(est.ORP.sin)
confint(est.ORP.sin, level=0.95)
  # Horrendous R2
...
confint(est.temp, level=0.95)
  # LINEARRRRR
xyplot(sin(Abundance) ~ Temperature..C., est.df)
est.temp.sin <- lm(Temperature..C.~sin(Abundance), est.df)
summary(est.temp.sin)
confint(est.temp.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.TDS, level=0.95)
  # LINEARRRRRR
xyplot(sin(Abundance) ~ TDS..mg.L., est.df)
est.TDS.sin <- lm(TDS..mg.L.~sin(Abundance), est.df)
summary(est.TDS.sin)
confint(est.TDS.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.NO3, level=0.95)
  # LINEARRRRRR
xyplot(sin(Abundance) ~ NO3.mg.L, est.df)
est.NO3.sin <- lm(NO3.mg.L~sin(Abundance), est.df)
summary(est.NO3.sin)
confint(est.NO3.sin, level=0.95)
  # Neg R2
...
confint(est.pH, level=0.95)
  # LINEARRRRRR
xyplot(sin(Abundance) ~ pH, est.df)
est.pH.sin <- lm(pH~sin(Abundance), est.df)
summary(est.pH.sin)
confint(est.pH.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.Cl, level=0.95)
  # LINEARRRRRR
xyplot(sin(Abundance) ~ Cl.mg.L, est.df)
est.Cl.sin <- lm(Cl.mg.L~sin(Abundance), est.df)
summary(est.Cl.sin)
confint(est.Cl.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.SRP, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ SRP.ug.L, est.df)
est.SRP.sin <- lm(SRP.ug.L~sin(Abundance), est.df)
summary(est.SRP.sin)
confint(est.SRP.sin, level=0.95)
  # Not linear
...
confint(est.Alk, level=0.95)
  # LINEARRRRR
xyplot(sin(Abundance) ~ Alk.mg.L, est.df)
est.Alk.sin <- lm(Alk.mg.L~sin(Abundance), est.df)
summary(est.Alk.sin)
confint(est.Alk.sin, level=0.95)
  # Not Linear
  # Neg R2
...
confint(est.Turb, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ Turb.NTU, est.df)
est.Turb.sin <- lm(Turb.NTU~sin(Abundance), est.df)
summary(est.Turb.sin)
confint(est.Turb.sin, level=0.95)
  # R2 essentially 0
...
confint(est.SpCond, level=0.95)
  # LINEARRRRR
xyplot(sin(Abundance) ~ SpCond...S.cm., est.df)
est.SpCond.sin <- lm(SpCond...S.cm.~sin(Abundance), est.df)
summary(est.SpCond.sin)
confint(est.SpCond.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.Chl, level=0.95)
  # Linear
xyplot(sin(Abundance) ~ Chl.Lab..ug.L., est.df)
est.Chl.sin <- lm(Chl.Lab..ug.L.~sin(Abundance), est.df)
summary(est.Chl.sin)
confint(est.Chl.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.BGA, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ BGA.cells.mL, est.df)
est.BGA.sin <- lm(BGA.cells.mL~sin(Abundance), est.df)
summary(est.BGA.sin)
confint(est.BGA.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.SO4, level=0.95)
  # Linear
xyplot(sin(Abundance) ~ SO4.mg.L, est.df)
est.SO4.sin <- lm(SO4.mg.L~sin(Abundance), est.df)
summary(est.SO4.sin)
confint(est.SO4.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.NH3, level=0.95)
  # Not linear
xyplot(sin(Abundance) ~ NH3.mg.L, est.df)
est.NH3.sin <- lm(NH3.mg.L~sin(Abundance), est.df)
summary(est.NH3.sin)
confint(est.NH3.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.TKN, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ TKN.mg.L, est.df)
est.TKN.sin <- lm(TKN.mg.L~sin(Abundance), est.df)
summary(est.TKN.sin)
confint(est.TKN.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(est.TP, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ TP.ug.L, est.df)
est.TP.sin <- lm(TP.ug.L~sin(Abundance), est.df)
summary(est.TP.sin)
confint(est.TP.sin, level=0.95)
  # Not linear
...
confint(est.DO, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ DO.probe.mg.L, est.df)
est.DO.sin <- lm(DO.probe.mg.L~sin(Abundance), est.df)
summary(est.DO.sin)
confint(est.DO.sin, level=0.95)
  # Not linear
  # Neg R2
...
xyplot(sin(Abundance) ~ SRP.ug.L, inl.df)
inl.SRP.sin <- lm(SRP.ug.L~sin(Abundance), inl.df)
summary(inl.SRP.sin)
confint(inl.SRP.sin, level=0.95)
  # Not linear
  # Neg R2
...
xyplot(sin(Abundance) ~ Chl.Lab..ug.L., inl.df)
inl.Chl.sin <- lm(Chl.Lab..ug.L.~sin(Abundance), inl.df)
summary(inl.Chl.sin)
confint(inl.Chl.sin, level=0.95)
  # Not linear
  # Neg R2
...
confint(inl.TP, level=0.95)
  # Linear
xyplot(sin(Abundance) ~ TP.ug.L, inl.df)
inl.TP.sin <- lm(TP.ug.L~sin(Abundance), inl.df)
summary(inl.TP.sin)
confint(inl.TP.sin, level=0.95)
...
confint(inl.NH4, level=0.95)
  # Linear
xyplot(sin(Abundance) ~ NH4.ug.L, inl.df)
inl.NH4.sin <- lm(NH4.ug.L~sin(Abundance), inl.df)
summary(inl.NH4.sin)
confint(inl.NH4.sin, level=0.95)
...
confint(inl.NO3, level=0.95)
  # Negative R2
xyplot(sin(Abundance) ~ NO3.mg.L, inl.df)
inl.NO3.sin <- lm(NO3.mg.L~sin(Abundance), inl.df)
summary(inl.NO3.sin)
confint(inl.NO3.sin, level=0.95)
...
xyplot(sin(Abundance) ~ TDP.ug.L, inl.df)
inl.TDP.sin <- lm(TDP.ug.L~sin(Abundance), inl.df)
summary(inl.TDP.sin)
confint(inl.TDP.sin, level=0.95)
  # Neg R2
...
ann.seas <- data.frame(lab=c("", "a", "a", ""),
                       Lake_Type = factor(c("Laurentian", "Estuary", "Estuary",
                                            "Inland"),
                                          levels = c("Laurentian", "Estuary",
                                                     "Inland")),
                       Season = factor(c("Spring", "Spring", "Summer", "Fall"),
                                         levels = c("Spring", "Summer", "Fall")))
...
inv.simp.seas.plot <- ggplot(inv.simp, aes(x=Lake_Type, y=mean, fill=Season)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=8),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  geom_text(data = ann.seas, aes(x = c(1.33,1,0.67,1), y=c(5.15,14,14,5.15), label = lab), size = 3, color="red")
...
# Pull out just Inverse Simpson
rich <- final.stats[which(final.stats$measure == "Observed Richness"),]


### Fraction ###
#rich.water <- rich[which(rich$Depth != "Sediment"),]
rich.frac.plot <- ggplot(rich, aes(x=Lake_Type, y=mean, fill=Fraction)) +
  geom_boxplot(position=position_dodge(1)) +
  scale_fill_manual(values = c("rosybrown","gold","saddlebrown")) +
  xlab("Lake Type") + ylab("Observed Richness") +
  facet_grid(~ Lake_Type, space = "free_x", scale="free") +
  theme(axis.title.x = element_text(face="bold", size=10),
        axis.text.x = element_text(angle=0, colour="black", size=8),
        axis.text.y = element_text(colour="black", size=8),
...
rich.seas.plot <- ggplot(rich, aes(x=Lake_Type, y=mean, fill=Season)) +
  geom_boxplot(position=position_dodge(1)) +
  #scale_fill_manual(values = c("indianred1","skyblue2","mediumpurple2")) +
  xlab("Lake Type") + ylab("Observed Richness") +
  facet_grid(~ Lake_Type, space = "free_x", scale="free") +
...
rich.depth.plot <- ggplot(rich, aes(x=Lake_Type, y=mean, fill=Depth)) +
  geom_boxplot(position=position_dodge(1)) +
  scale_fill_manual(values = c("cadetblue1","dodgerblue3","purple3")) +
  xlab("Lake Type") + ylab("Observed Richness") +
  facet_grid(~ Lake_Type, space = "free_x", scale="free") +
...
##### Adjust P-values #####
invsimp.kw.vals <- data.frame(c(0.1772, 0.0005339, 0.6899, 0.1208, 0.002485, 0.0007338, 0.3115, 0.002353, 0.001522, 0.7604, 0.4207, 0.1345, 0.01171, 0.02461, 0.4749, 0.005921, 0.364, 0.00115, 0.09118, 0.03019, 0.00213, 0.3071, 0.004267, 0.0008396, 0.9449))
...
# Lake Type
kruskal.test(formula = mean ~ Lake_Type, data = rich)
  # p-value = 0.004742

# Source
kruskal.test(formula = mean ~ Source, data = rich)
  # p-value = 1.337e-06

# Free vs Part
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Fraction != "Sediment")
  # p-value = 0.0002538

### Fraction

# Laurentian
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Laurentian")
  # Fraction
  # p-value = 0.006467

# Estuary
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Estuary")
  # Fraction
  # p-value = 4.155e-06
kruskalmc(resp = mean ~ Fraction, data = rich, subset=Lake_Type=="Estuary")
  # All sig
kruskal.test(formula = mean ~ Source, data = rich, subset=Lake_Type=="Estuary")
  # Sed vs Water
  # p-value = 5.393e-06
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Estuary" & Fraction != "Sediment")
  # Free vs Part
  # p-value = 0.02801
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Estuary" & Fraction != "Free")
  # Part vs Sed
  # p-value = 0.000177
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Estuary" & Fraction != "Particle")
  # Free vs Sed
  # p-value = 3.859e-06

# Inland
kruskal.test(formula = mean ~ Fraction, data = rich, subset=Lake_Type=="Inland")
  # Fraction
  # p-value = 0.04954

### Season
kruskal.test(formula = mean ~ Season, data = rich)
  # All Season
  # p-value = 0.5842

# Laurentian
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Laurentian")
  # Season  
  # p-value = 0.01251

# Estuary
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Estuary")
  # Season
  # p-value = 0.6271
kruskalmc(resp = mean ~ Season, data = rich, subset=Lake_Type=="Estuary")
  # No sig diff?
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Estuary" & Season != "Spring")
  # Summer vs Fall
  # p-value = 0.4455
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Estuary" & Season != "Summer")
  # Spring vs Fall
  # p-value = 0.3825
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Estuary" & Season != "Fall")
  # Spring vs Summer
  # p-value = 0.8738

# Inland
kruskal.test(formula = mean ~ Season, data = rich, subset=Lake_Type=="Inland")
  # Season
  # p-value = 0.1804


### Depth
kruskal.test(formula = mean ~ Depth, data = rich)
  # All Depth
  # p-value = 1.056e-06
kruskalmc(resp = mean ~ Depth, data = rich)
  # Sur-Sed, Bot-Sed = Sig
  # Sur-Bot = nonsig
kruskal.test(formula = mean ~ Depth, data = rich, subset= Depth != "Sediment")
  # All Water
  # p-value = 0.03532

# Laurentian
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Laurentian")
  # Depth 
  # p-value = 0.03603

# Estuary
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Estuary")
  # Depth
  # p-value = 1.69e-05
kruskalmc(resp = mean ~ Depth, data = rich, subset=Lake_Type=="Estuary")
  # Sur-Bot = nonsig
  # Sur-Sed, Bot-Sed = Sig
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Estuary" & Depth != "Sediment" )
  # Sur vs Bottom
  # p-value = 0.1971
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Estuary" & Depth != "Surface" )
  # Bottom vs Sediment
  # p-value = 5.803e-05
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Estuary" & Depth != "Bottom" )
  # Sur vs Sediment 
  # p-value = 1.437e-05

# Inland
kruskal.test(formula = mean ~ Depth, data = rich, subset = Lake_Type == "Inland")
  # p-value = 0.4346

##### Adjust P-values #####
rich.kw.vals <- data.frame(c(0.004742, 1.337e-06, 0.0002538, 0.006467, 4.155e-06, 5.393e-06, 0.02801, 0.000177, 3.859e-06, 0.04954, 0.5842, 0.01251, 0.6271, 0.4455, 0.3825, 0.8738, 0.1804, 1.056e-06, 0.03532, 0.03603, 1.69e-05, 0.1971, 5.803e-05, 1.437e-05, 0.4346))
colnames(rich.kw.vals) <- "pvals"
rich.kw.vals$Lake <- c("All", "All", "All", "L", "E", "E", "E", "E", "E", "I", "All", "L", "E", "E", "E", "E", "I", "All", "All", "L", "E", "E", "E", "E", "I")
rich.kw.vals$Comp <- c("Lake", "Source", "PF", "Frac", "Frac", "Source", "PF", "PS", "FS", "PF", "Seas", "Seas", "Seas", "SuFa", "SpFa", "SpSu", "Seas", "Depth", "TB", "TB", "Depth", "TB", "BS", "TS", "Depth")
rich.kw.vals$Bonf <- p.adjust(rich.kw.vals$pvals, method="bonferroni")
rich.kw.vals$BenH <- p.adjust(rich.kw.vals$pvals, method="BH")
# Sig = All Lake, All Source, All PF, Lau Frac, Est Frac, Est Source, Lau seas
...
dev.new()
png("boxplot.invsimp.png", width = 17.8, height = 10, units = "cm", res = 600)
grid.newpage()
...
# Calculate MDP (Mean Pairwise Distance)
#ses.mpd.result.weigh <- ses.mpd(com, phydist.weigh, null.model = "taxa.labels", 
 #                               abundance.weighted=T, runs=100)
#
## Calculate MNTD (Mean Nearest Taxon Distance)
...
#weigh.stats <- rbind(pd.w.stats, mpd.w.stats)
...
#weigh.stats <- rbind(weigh.stats, mpd.w.stats)
...
#final.weigh$Measure <- factor(final.weigh$Measure, levels = c("PD", "MPD SES", "MPD p-value"))
...
#pd.all <- rbind(pd.stats, pd.w.stats)
  # This has the unweighted PD
#s <- data.frame(sample_data(Ver)) 
pd.ready <- merge(pd.stats, s, by="SampleID")
...
pd.frac.plot <- ggplot(final.pd, aes(x=Lake_Type, y=Stat, fill=Fraction)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=10),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  geom_text(data = ann.pd.frac, aes(x = c(1,1.33,1), y=c(2.5, 2.5, 2.5), label = lab), size = 4, color="red")



### Season ###
ann.pd.seas <- data.frame(lab=c("+", " ", "", ""),
                       Lake_Type = factor(c("Laurentian", "Estuary", "Inland",
                                            "Inland"),
                                          levels = c("Laurentian", "Estuary",
                                                     "Inland")),
...
pd.seas.plot <- ggplot(final.pd, aes(x=Lake_Type, y=Stat, fill=Season)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=10),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  geom_text(data = ann.pd.seas, aes(x = c(1.33,1,0.67,1), y=c(2.5,2.5,2.5,2.5), label = lab), size = 4, color="red")
...
##### ISOLATE MPD #####
#mpd.all <- rbind(mpd.stats, mpd.w.stats)
#s <- data.frame(sample_data(Ver))
...
######## Plot PD & MPD ########
tiff("pd.mpd.tiff", width = 17.8, height = 14, units = "cm", res = 600)
grid.newpage()
pushViewport(viewport(layout=(grid.layout(2,2, widths=c(1,1), heights=c(0.5,0.5))))) 
print(pd.frac.plot, vp=viewport(layout.pos.row=1, layout.pos.col=1))
print(pd.seas.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
print(mpd.frac.plot, vp=viewport(layout.pos.row=2, layout.pos.col=1))
print(mpd.seas.plot, vp=viewport(layout.pos.row=2, layout.pos.col=2))
dev.off()
...
kruskal.test(formula = Stat ~ Lake_Type, data = final.pd)
  # All Lake Types
  # p-value = 0.1137
kruskalmc(resp = Stat ~ Lake_Type, data = final.pd)
  # No sig
...
kruskal.test(formula = Stat ~ Fraction, data = final.pd)
  ...
  # p-value = 3.932e-09
# Source
kruskal.test(formula = Stat ~ Source, data=final.pd)
  # All Source
  # p-value = 4.269e-06
kruskalmc(resp = Stat ~ Fraction, data = final.pd)
  # P-F = nonsig
  # Water vs. sed = sig
...
kruskal.test(formula = Stat ~ Fraction, data = final.pd, subset = Lake_Type == "Estuary")
  # Fraction
  # p-value = 1.308e-06
kruskalmc(resp = Stat ~ Fraction, data = final.pd, subset = Lake_Type == "Estuary")
  # P-F = nonsig
  # Water vs. sed = sig
kruskal.test(formula = Stat ~ Source, data = final.pd, subset = Lake_Type == "Estuary")
  # Source
  # p-value = 1.374e-06
...
kruskal.test(formula = Stat ~ Season, data = final.pd, subset = Lake_Type == "Laurentian")
  # Season
  # p-value = 3.045e-05
kruskalmc(resp = Stat ~ Season, data = final.pd, subset = Lake_Type=="Laurentian")
  # No diff?
...
kruskal.test(formula = Stat ~ Depth, data = final.pd)
  # All Depth
  # p-value = 2.332e-05
kruskalmc(resp = Stat ~ Depth, data = final.pd)
  # Sur-Sed, Bot-Sed = Sig
  # Sur-Bot = nonsig
...
kruskal.test(formula = Stat ~ Depth, data = final.pd, subset = Lake_Type == "Estuary")
  # Depth
  # p-value = 7.646e-06
kruskalmc(resp = Stat ~ Depth, data = final.pd, subset = Lake_Type=="Estuary")
  # Sur-Sed, Bot-Sed = Sig
  # Sur-Bot = Nonsig
kruskal.test(formula = Stat ~ Source, data = final.pd, subset = Lake_Type == "Estuary")
  # Source
...
kruskal.test(formula = Stat ~ Depth, data = final.pd, subset = Lake_Type == "Estuary" & Depth != "Sediment")
  # Sur vs Bottom
  # p-value = 0.5619
kruskal.test(formula = Stat ~ Depth, data = final.pd, subset = Lake_Type == "Estuary" & Depth != "Surface")
  # Bottom vs Sediment
  # p-value = 1.153e-05
...
##### Adjust P-Values #####
pd.kw.vals <- data.frame(c(0.1137, 0.1192, 0.07596, 0.7314, 3.932e-09, 4.269e-06, 0.03951, 0.09885, 1.308e-06, 1.374e-06, 0.02507, 3.859e-06, 1.999e-05, 0.4808, 0.8721, 3.045e-05, 0.0001352, 8.607e-05, 0.3557, 0.822, 0.08661, 0.2661, 0.02478, 0.08827, 2.332e-05, 0.6679, 0.3743, 7.646e-06, 1.374e-06, 0.5619, 1.153e-05, 6.985e-06, 0.5098))
colnames(pd.kw.vals) <- "pvals"
pd.kw.vals$Lake <- c("All", "EI", "LI", "LE", "All", "All", "All", "L", "E", "E", "E", "E", "E", "I", "All", "L", "L", "L", "L", "E", "I", "I", "I", "I", "All", "All", "L", "E", "E", "E", "E", "E", "I")
pd.kw.vals$Comp <- c("Lake", "EI", "LI", "LE", "Frac", "Source", "PF", "Frac", "Frac", "Source", "PF", "FS", "PS", "Frac", "Seas", "Seas", "SuFa", "SpFa", "Spsu", "Seas", "Seas", "SuFa", "SpFa", "SpSu", "Depth", "TB", "Depth", "Depth", "Source", "TB", "BS", "TS", "Depth")
pd.kw.vals$Bonf <- p.adjust(pd.kw.vals$pvals, method="bonferroni")
pd.kw.vals$BenH <- p.adjust(pd.kw.vals$pvals, method="BH")
...
##### Adjust P-Values #####
mpd.kw.vals <- data.frame(c(3.611e-06, 0.001532, 0.005122, 0.7314, 6.903e-14, 7.149e-08, 5.05e-09, 0.02699, 1.543e-06, 5.134e-07, 0.1585, 1.755e-06, 8.267e-06, 1.72e-08, 0.7346, 0.003229, 0.05634, 0.001819, 0.05628, 0.8828, 0.02586, 0.5843, 0.03064, 0.009947, 4.23e-07, 0.7764, 0.1021, 2.083e-06, 5.134e-07, 0.2426, 4.527e-06, 3.316e-06, 0.1965))
colnames(mpd.kw.vals) <- "pvals"
mpd.kw.vals$Lake <- c("All", "EI", "LI", "LE", "All", "All", "All", "L", "E", "E", "E", "E", "E", "I", "All", "L", "L", "L", "L", "E", "I", "I", "I", "I", "All", "All", "L", "E", "E", "E", "E", "E", "I")
mpd.kw.vals$Comp <- c("Lake", "EI", "LI", "LE", "Frac", "Source", "PF", "Frac", "Frac", "Source", "PF", "FS", "PS", "Frac", "Seas", "Seas", "SuFa", "SpFa", "Spsu", "Seas", "Seas", "SuFa", "SpFa", "SpSu", "Depth", "TB", "Depth", "Depth", "Source", "TB", "BS", "TS", "Depth")
...
Ver.est.ord <- plot_ordination(physeq = Ver.est, ordination = Ver.est.pcoa, axes = c(1, 2),
  ...
  theme(axis.text.x = element_blank(), 
        ...
        plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"), #top, right, bottom, left
        plot.title = element_text(size = 9, face="bold")) +
  annotate("text", x= -0.31, y=0.41, label = "B", fontface = "bold", size=5, colour="black") +
  guides(colour=FALSE, shape=FALSE, alpha = FALSE) 
  #guides(color = guide_legend(order=1, title="Season", override.aes=list(size=3)),
  #       shape = guide_legend(order=2, title="Depth",override.aes=list(size=3)),
  #       alpha = guide_legend(order=3, title="Fraction",override.aes=list(size=3)))
...
scaled.est.ord <- plot_ordination(physeq = scaled.est, ordination = scaled.est.pcoa, axes = c(1, 2),
  ...
  theme(axis.text.x = element_blank(), 
        ...
        plot.title = element_text(size = 9, face="bold")) +
  scale_y_reverse() +
  annotate("text", x= -0.26, y=-0.26, label = "E", fontface = "bold", size=5, colour="black") +
  guides(colour=FALSE, shape=FALSE, alpha = FALSE) 
  #guides(color = guide_legend(order=1, title="Season", override.aes=list(size=3)),
  #       shape = guide_legend(order=2, title="Depth",override.aes=list(size=3)),
  #       alpha = guide_legend(order=3, title="Fraction",override.aes=list(size=3)))
...
### Check Community with Verrucos removed

# Remove Verrucos
no.ver <- subset_taxa(scaled, Phylum != "Verrucomicrobia")
  
# Calculate bray-curtis dissimilarity
no.ver.bray <- phyloseq::distance(physeq=no.ver, method="bray")

# Perform PCoA ordination
no.ver.pcoa <- ordinate(physeq = no.ver, method = "PCoA", distance="bray")

procrust.no.ver <- procrustes(no.ver.pcoa$vectors[,1:2], Ver.pcoa$vectors[,1:2],
                              scale=F, symmetric=F)
  # Sum of squares = 10.06
pro.no.ver <- protest(no.ver.pcoa$vectors[,1:2], Ver.pcoa$vectors[,1:2])
  # p-value = 0.001
  # m12^2 (sum of squares) = 0.4105
  # correlation = 0.7678
...
#   scale_color_brewer (palette="Set1") +
#   ggtitle("Surface (Top) vs. Bottom (Bottom)")
...
#   scale_color_brewer(palette="Set1") +
#   ggtitle("Surface (Top) vs. Sediment (Bottom)")
...
# Read in metadata files
lau.part.env <- read.table("metadata.lau.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
lau.free.env <- read.table("metadata.lau.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
est.part.env <- read.table("metadata.est.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
est.part.env <- est.part.env[,-27]
  # Remove DO %
est.free.env <- read.table("metadata.est.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
est.free.env <- est.free.env[,-27]
  # Remove DO %
inl.part.env <- read.table("metadata.inl.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
inl.free.env <- read.table("metadata.inl.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
...
# Read in metadata files
scaled.lau.part.env <- read.table("metadata.lau.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
scaled.lau.free.env <- read.table("metadata.lau.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
scaled.est.part.env <- read.table("metadata.est.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
scaled.est.part.env <- scaled.est.part.env[,-27]
  # Remove DO %
scaled.est.free.env <- read.table("metadata.est.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
scaled.est.free.env <- scaled.est.free.env[,-27]
  # Remove DO %
scaled.inl.part.env <- read.table("metadata.inl.part.csv", header=T, sep=",", row.names=1,
                      as.is=T)
scaled.inl.free.env <- read.table("metadata.inl.free.csv", header=T, sep=",", row.names=1,
                      as.is=T)
