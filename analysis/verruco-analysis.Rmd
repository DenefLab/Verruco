...
lau.otus <- data.frame(t(otu_table(subset_samples(meta.otus,
                                                  Lake_Type=="Laurentian"))))
est.otus <- data.frame(t(otu_table(subset_samples(meta.otus,
                                                  Lake_Type=="Estuary"))))
inl.otus <- data.frame(t(otu_table(subset_samples(meta.otus
                                                  , Lake_Type=="Inland"))))
...
# Trim otu tables so that they match metadata samples
# Laurentian
lau.otus <- subset
...
# Run Bioenv
bioenv.lau <- bioenv(lau.otus ~ Temperature..C. + SpCond...S.cm. + DO.Probe..mg.L. + Chl.Probe...g.L. + Chl.Lab..ug.L. + TP.ug.L + PP.ug.L + TDP.ug.L + TDP.SRP + POC.mg.L + PON.mg.L + DOC.mg.L + SiO2.mg.L + TSS.mg.L + Fluorescence.mg.m3. + PAR, lau.env, method = "spearman", index = "bray", trace = TRUE, partial = NULL)



bioenv.inl <- bioenv(inl.otus ~ Chl.Lab..ug.L. + NO3.mg.L + SRP.ug.L + TP.ug.L + TDP.ug.L + NH4.ug.L, inl.env, method = "spearman", index = "bray", trace = TRUE, partial = NULL)

bioenv.inl <- bioenv(comm = inl.otus, env = test.env)
