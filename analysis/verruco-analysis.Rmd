...
## Laurentian
xyplot(Abundance ~ DO.Probe..mg.L., lau.trim)
lau.DO <- lm(DO.Probe..mg.L.~Abundance, lau.trim)
summary(lau.DO)
confint(lau.DO, level=0.95)
  # Linear

xyplot(Abundance ~ TP.ug.L, lau.trim)
lau.TP <- lm(TP.ug.L~Abundance, lau.trim)
summary(lau.TP)
confint(lau.TP, level=0.95)
  # Not linear, but R2 is relatively high-ish (.1019)

xyplot(Abundance ~ PON.mg.L, lau.trim)
lau.PON <- lm(PON.mg.L~Abundance, lau.trim)
summary(lau.PON)
confint(lau.PON, level=0.95)
  # Not linear

xyplot(Abundance ~ DOC.mg.L, lau.trim)
lau.DOC <- lm(DOC.mg.L~Abundance, lau.trim)
summary(lau.DOC)
confint(lau.DOC, level=0.95)
  # Linear

xyplot(Abundance ~ PAR, lau.trim)
lau.PAR <- lm(PAR~Abundance, lau.trim)
summary(lau.PAR)
confint(lau.PAR, level=0.95)
  # Not linear
  # How the hell does one get a negative R2?


## Estuary
xyplot(Abundance ~ ORP..mV., est.trim)
est.ORP <- lm(ORP..mV.~Abundance, est.trim)
summary(est.ORP)
confint(est.ORP, level=0.95)
  # Linear

xyplot(Abundance ~ Chl.Lab..ug.L., est.trim)
est.Chl <- lm(Chl.Lab..ug.L.~Abundance, est.trim)
summary(est.Chl)
confint(est.Chl, level=0.95)
  # Linear

xyplot(Abundance ~ BGA.cells.mL, est.trim)
est.BGA <- lm(BGA.cells.mL~Abundance, est.trim)
summary(est.BGA)
confint(est.BGA, level=0.95)
  # Not linear

xyplot(Abundance ~ SO4.mg.L, est.trim)
est.SO4 <- lm(SO4.mg.L~Abundance, est.trim)
summary(est.SO4)
confint(est.SO4, level=0.95)
  # Linear

xyplot(Abundance ~ NH3.mg.L, est.trim)
est.NH3 <- lm(NH3.mg.L~Abundance, est.trim)
summary(est.NH3)
confint(est.NH3, level=0.95)
  # Linear?

xyplot(Abundance ~ TKN.mg.L, est.trim)
est.TKN <- lm(TKN.mg.L~Abundance, est.trim)
summary(est.TKN)
confint(est.TKN, level=0.95)
  # asdfasdf
  # Again with the negative R2

xyplot(Abundance ~ TP.ug.L, est.trim)
est.TP <- lm(TP.ug.L~Abundance, est.trim)
summary(est.TP)
confint(est.TP, level=0.95)
  # asdfash
  # Again with the negative R2

xyplot(Abundance ~ DO.probe.mg.L, est.trim)
est.DO <- lm(DO.probe.mg.L~Abundance, est.trim)
summary(est.DO)
...
## Inland
xyplot(Abundance ~ NO3.mg.L, inl.trim)
inl.NO3 <- lm(NO3.mg.L~Abundance, inl.trim)
summary(inl.NO3)
confint(inl.NO3, level=0.95)
  # Linear

xyplot(Abundance ~ TP.ug.L, inl.trim)
inl.TP <- lm(TP.ug.L~Abundance, inl.trim)
summary(inl.TP)
confint(inl.TP, level=0.95)
  # Linear?

xyplot(Abundance ~ NH4.ug.L, inl.trim)
inl.NH4 <- lm(NH4.ug.L~Abundance, inl.trim)
summary(inl.NH4)
...
bs.tre <- plotBS(midpoint(fit.optim$tree), bs, p = 50, type="p")
