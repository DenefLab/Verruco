...
sample_data(scaled)$Group <- ordered(sample_data(scaled)$Group,
...
### Facet = Group, Color = Depth, Shape = Fraction
...
#test <- phyloseq_to_deseq2(Ver1, design= ~Group+Season+Source)
...
source <- deSEQ(data=Ver1, valuetest= ~Group+Season+Source)
...
watercol <- deSEQ(data=Ver.water, valuetest= ~Group+Season+Depth, alpha=0.05)
...
Vbs <- deSEQ(data=Ver.bs, valuetest= ~Group+Season+Depth, alpha=0.05)
...
Vst <- deSEQ(data=Ver.st, valuetest= ~Group+Season+Depth, alpha=0.05)
...
V.el <- subset_samples(Ver1, Group != "Inland")
Vel <- deSEQ(data=V.el, valuetest= ~Depth+Season+Group, alpha=0.05)
...
V.ei <- subset_samples(Ver1, Group != "Laurentian")
Vei <- deSEQ(data=V.ei, valuetest= ~Depth+Season+Group, alpha=0.05)
...
V.li <- subset_samples(Ver1, Group != "Estuary")
Vli <- deSEQ(data=V.li, valuetest= ~Depth+Season+Group, alpha=0.05)
...
tiff("deseq.group.tiff", width = 20, height = 10, units = "in", res = 500)
grid.newpage()
...
Vfrac <- deSEQ(data=V.frac, valuetest= ~Depth+Group+Season+Fraction, alpha=0.05)
...
Vss <- deSEQ(data=V.ss, valuetest= ~Depth+Group+Season, alpha=0.05)
...
Vspf <- deSEQ(data=V.spf, valuetest= ~Depth+Group+Season, alpha=0.05)
...
Vsuf <- deSEQ(data=V.suf, valuetest= ~Depth+Group+Season, alpha=0.05)
...
Group.label <- rownames(tax_table(Ver1))
...
write.csv(group.split[5,], file="Ecology_Otu4", row.names=FALSE, quote=FALSE)
```

### Create updated phyloseq object with EcoID
```{r ecoid.phyloseq}
##### Create new phyloseq object with EcoIDs
eco.label<- read.csv("Ecology_Otu4", header=F)
...
write.csv(color.all$label,file="all.ver.color4",row.names=FALSE,quote=FALSE)
...
write.csv(ecoid.all$itol.label,"EcoID.IToL.label4", row.names=FALSE, quote=FALSE)
```
...
write.tree(phy_tree(only.sig), file="ver.sig.tre4")
