...
# Set phylum plotting colors for 8 taxa
phylum.colors <- c(Actinobacteria="firebrick3",
...
##### PLOT #####
tiff("phy.relabund.tiff", width = 10, height = 10, units = "cm", res = 600)
ggplot(phy.trim, aes(x=Phylum, y=Median, fill=Phylum)) +
  ...
  facet_grid(Lake_Type~., space="free", scales = "free_x") +
...
phy.lau.plot<- ggplot(phy.lau.sum, aes(x=Phylum, y=Median, fill=Phylum)) +
          geom_bar(stat="identity", position="dodge", color="black") +
          #scale_y_continuous(labels = percent_format()) +
          scale_fill_manual(values = phylum.colors, name="Phylum") +
  geom_errorbar(aes(ymax=Median+SE, ymin=Median-SE), width=0.3) + 
  ...
  scale_y_continuous(breaks = c(0,5,10,15,20,25,30,35,40), limits = c(0,40)) +
          theme(axis.title.x = element_blank(),
                ...
                legend.text = element_text(size =8),
                legend.position="right") +
  guides(fill=F) + xlab("Phyla") + ylab("Laurentian")
...
#dev.off()
 ...
 ##### Let's steal the legend 
  g_legend <- function(a.gplot){
    ...
    return(legend)}
  legend <- g_legend(phy.lau.plot)


##### Estuary #####
#tiff("stackbar.Lake_Type.tiff", width = 7, height = 8, units = "in", res = 500)
phy.est.plot <- ggplot(phy.est.sum, aes(x=Phylum, y=Median, fill=Phylum)) +
          geom_bar(stat="identity", position="dodge", color="black") +
          #scale_y_continuous(labels = percent_format()) +
          scale_fill_manual(values = phylum.colors, name="Phylum") +
  geom_errorbar(aes(ymax=Median+SE, ymin=Median-SE), width=0.3) + 
  ...
  scale_y_continuous(breaks = c(0,5,10,15,20,25,30,35,40), limits = c(0,40)) +
          theme(axis.title.x = element_blank(),
                ...
                legend.position="right") +
          #guides(fill = guide_legend(keywidth = 1, keyheight = 1)) +
          guides(fill=F) + xlab("Phyla") + ylab("Estuary")
#dev.off()


##### Inland #####
#tiff("stackbar.Lake_Type.tiff", width = 7, height = 8, units = "in", res = 500)
phy.inl.plot <- ggplot(phy.inl.sum, aes(x=Phylum, y=Median, fill=Phylum)) +
          geom_bar(stat="identity", position="dodge", color="black") +
          #scale_y_continuous(labels = percent_format()) +
          scale_fill_manual(values = phylum.colors, name="Phylum") +
  geom_errorbar(aes(ymax=Median+SE, ymin=Median-SE), width=0.3) + 
  ...
  scale_y_continuous(breaks = c(0,5,10,15,20,25,30,35,40), limits = c(0,40)) +
          theme(axis.title.x = element_text(size=10, face = "bold"),
                ...
                legend.position="right") +
          #guides(fill = guide_legend(keywidth = 1, keyheight = 1)) +
          guides(fill=F) + xlab("Phyla") + ylab("Inland")
#dev.off()


##### Make big plot #####
tiff("phy.relabund.tiff", width = 8.7, height = 10, units = "cm", res = 600)
grid.newpage()
pushViewport(viewport(layout=(grid.layout(3,3, widths=c(0.4,7,3.5), heights=c(1,1,1.13)))))
grid.text("Relative Abundance (Taxa > 1%)", rot=90,
          vp=viewport(layout.pos.row=c(1,3), layout.pos.col = 1),
                      gp=gpar(fontsize=10, fontface="bold"))
print(phy.lau.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
print(phy.est.plot, vp=viewport(layout.pos.row=2, layout.pos.col=2))
print(phy.inl.plot, vp=viewport(layout.pos.row=3, layout.pos.col=2))
legend$vp <- viewport(layout.pos.row = 2, layout.pos.col = 3)
grid.draw(legend)
dev.off()
...
seas.box <- ggplot(Ver.box, aes(x=Lake_Type, y=Relative_Abundance, fill=Season)) +
  ...
  theme(axis.title.x = element_text(face="bold", size=10),
        ...
        strip.text.x = element_blank(),
        plot.margin = unit(c(0.1, -0.1, 0.1, 0.1), "cm")) + # top, right, bottom, left
  geom_text(data = ann_text, aes(label=lab), x=c(1,1,1), y=c(41,41,41), size=6, color="red")

ann_text <- data.frame(lab=c(" ", " ", "*"), Lake_Type = factor(c("Laurentian", "Estuary", "Inland"), levels = c("Laurentian", "Estuary", "Inland")), Season = factor(c("Spring", "Summer", "Fall")))
...
### Test Fraction within Lake Type ###
...
```{r}
png("boxplot.invsimp.png", width = 17.8, height = 15, units = "cm", res = 600)
grid.newpage()
...
print(inv.simp.plot, vp=viewport(layout.pos.row=2, layout.pos.col=1:2))
dev.off()
