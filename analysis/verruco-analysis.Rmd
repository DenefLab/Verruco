...
est.best.model <- lapply(est.df, 
  function(x) {
    best_model <- get_bestsub_summary(est.vars2, "Abundance", est.df)
    model <- lm(
      formula = reformulate(est.vars2[best_model$model[-1]], "Abundance"), 
      data = est.df)
    return(model)
  })
...
inl.best.model <- lapply(inl.df, 
  function(x) {
    best_model <- get_bestsub_summary(inl.vars2, "Abundance", inl.df)
    model <- lm(
      formula = reformulate(inl.vars2[best_model$model[-1]], "Abundance"), 
      data = inl.df)
    return(model)
  })
