...
kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est)
  # All Stations
  # p-value = 0.008687
mc.test <- kruskalmc(resp = Relative_Abundance ~ Station, data = Ver.box.est)
...
test.buoy <- which(Ver.box.est$Station == "Buoy")
test.out <- which(Ver.box.est$Station == "Outlet")
test.df <- Ver.box.est[c(test.buoy, test.out),]
test.pair <- kruskal.test(formula = Relative_Abundance ~ Station, data = test.df)

test <- kruskal.test(formula = Relative_Abundance ~ Station, data = Ver.box.est,
             subset = Station != c("Inlet", "Bear", "Bottom"))
